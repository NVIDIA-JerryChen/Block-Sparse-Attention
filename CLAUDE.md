...
blk64 additionally requires CUTLASS headers (path configured in `csrc/fwd/sm100_blk64/setup.py`).
...
- `bindings.cpp` — PyTorch C++ extension bindings (`bsa_fwd_blk64_ext`)
- `setup.py` — Build via `torch.utils.cpp_extension.CUDAExtension` (SM100a target)
