...
- `q2k_block_index`: `(batch, num_heads, num_q_blocks, max_kv_blocks)` int32 — per Q-block list of KV block indices to attend to (first `block_sparse_num` entries valid)
- `block_sparse_num`: int (runtime, even, >= 2) — number of KV blocks each Q block attends to
