...
class BucketedK2QCsrUniversal:
    ...
    def __init__(
        ...
        self.num_edge_tiles = (
            self.edge_width + self.num_edge_threads - 1
    ...
    def _count_edges_kernel(
        ...
        if block_idx < self.edge_width:
            ...
            if block_idx < num_active_blocks:
                kv_block_idx = mQ2kBlockIndex[
                    batch_idx, head_idx, q_block_idx, block_idx
                ]
                if kv_block_idx >= 0:
    ...
    def _finalize_offsets_kernel(
        ...
        for kv_block_idx in cutlass.range(num_kv_blocks, unroll=1):
            offset = (
                group_base
                + mLocalOffsets[batch_idx, head_idx, q_group_idx, kv_block_idx]
            )
        ...
        mBucketedK2qOffsets[batch_idx, head_idx, q_group_idx, num_kv_blocks] = (
            group_base + mLocalOffsets[batch_idx, head_idx, q_group_idx, num_kv_blocks]
        )
            ...
            if block_idx < num_active_blocks:
                ...
                if kv_block_idx >= 0:
                    if kv_block_idx < num_kv_blocks:
                        ...
                        mBucketedK2qIndices[batch_idx, head_idx, position] = Int32(
                            q_block_idx
                        )
    ...
    *,
    ...
    current_stream = (
        ...
        if is_fake_mode()
            ...
            torch.cuda.current_stream(q2k_block_index.device).cuda_stream
        )
    )
    tensors = (
        counts,
...
build_bucketed_k2q_csr_cutedsl.compile_cache = get_jit_cache("bucketed_k2q_csr_cutedsl")
