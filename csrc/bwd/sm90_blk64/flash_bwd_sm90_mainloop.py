...
# BSA-local copy of BSA SM90 backward mainloop.
# Keep this file structurally close to the FA SM90 backward mainloop while
# incrementally adapting it for blk64 block-sparse Hopper backward.
...
from csrc.bwd.sm90_blk64.cute_dsl_utils import assume_tensor_aligned
from csrc.bwd.sm90_blk64 import utils
from csrc.bwd.sm90_blk64.mask import AttentionMask
from csrc.bwd.sm90_blk64.seqlen_info import SeqlenInfoQK
from csrc.bwd.sm90_blk64.block_info import BlockInfo
from csrc.bwd.sm90_blk64 import pipeline
from quack.cute_dsl_utils import ParamsBase
from csrc.bwd.sm90_blk64.tile_scheduler import (
    TileSchedulerArguments,
    SingleTileScheduler,
    SingleTileLPTBwdScheduler,
    SingleTileVarlenScheduler,
)
from csrc.bwd.sm90_blk64 import barrier
from csrc.bwd.sm90_blk64.named_barrier import NamedBarrierBwd
from csrc.bwd.sm90_blk64.softmax import apply_score_mod_inner, apply_score_mod_bwd_inner
from csrc.bwd.sm90_blk64.block_sparsity import BlockSparseTensors
from csrc.bwd.sm90_blk64.block_sparse_utils import (
...
class FlashAttentionBackwardSm90:
    ...
    def __init__(
        ...
        wg_specialized_pipeline: bool = False,
        ...
        assert qhead_per_kvhead == 1, "SM90 blk64 BSA bwd does not support GQA/MQA yet"
        assert not is_causal, "SM90 blk64 BSA bwd currently supports non-causal masks only"
        ...
        assert not V_in_regs or (
            SdP_swapAB and wg_specialized_pipeline
        ), "SM90 blk64 BSA bwd KV-in-regs requires dense WG-specialized SdP_swapAB"
        assert score_mod is None and score_mod_bwd is None, "score_mod is not in the BSA SM90 scope"
        assert mask_mod is None, "mask_mod is not in the BSA SM90 scope"
        assert not has_aux_tensors, "aux tensors are not in the BSA SM90 scope"
        assert subtile_factor == 1, "SM90 blk64 BSA bwd uses one sparse subtile per block"
        self.dtype = dtype
        ...
        self.mma_dkv_is_rs = (
            ...
            and SdP_swapAB
            and not dKV_swapAB
            and not wg_specialized_pipeline
        )
        ...
        # dQ_single_wg: first MMA WG computes the full dQ GEMM, second MMA WG skips it.
        ...
        self.wg_specialized_pipeline = wg_specialized_pipeline
        ...
        self.dKV_accum_output = dKV_accum_output
        ...
        if qbucket:
            assert wg_specialized_pipeline, "SM90 qbucket bwd uses the WG-specialized pipeline"
            assert dKV_accum_output, "SM90 qbucket bwd must accumulate dK/dV in fp32 workspace"
    ...
    def _check_type(
        ...
        if const_expr(self.qhead_per_kvhead == 1 and not self.dKV_accum_output):
    ...
    def _setup_attributes(self):
        ...
        wg_n_SdP = (
            1
            if const_expr(self.wg_specialized_pipeline)
            else self.num_wg_mma // self.AtomLayoutMSdP
        )
        wg_n_dKV = 1 if const_expr(self.wg_specialized_pipeline) else self.AtomLayoutNdKV
    ...
    def _get_tiled_mma(self):
        ...
        if self.wg_specialized_pipeline:
            ...
            tiled_mma_dK, tiled_mma_dV = [
                sm90_utils_basic.make_trivial_tiled_mma(
                    ...
                    self.dtype,
                    warpgroup.OperandMajorMode.K
                    if self.dKV_rs_wg1 or self.dKV_rs_split
    ...
    def _get_shared_storage_cls(self):
        ...
        if const_expr(self.dKV_accum_output):
            cosize_sK = max(cosize_sK, self.tile_n * self.tile_hdim * Float32.width // self.dtype.width)
            cosize_sV = max(
                cosize_sV, self.tile_n * self.tile_hdimv * Float32.width // self.dtype.width
            )
    ...
    def __call__(
        ...
        if const_expr(self.qbucket):
            assert qbucket_task_offsets is not None
            assert qbucket_task_q_indices is not None
            qbucket_task_offsets = cute.make_tensor(
                qbucket_task_offsets.iterator,
                cute.group_modes(
                    cute.select(qbucket_task_offsets.layout, mode=[3, 2, 1, 0]),
                    2,
                    4,
                ),
            )
            qbucket_task_q_indices = cute.make_tensor(
                qbucket_task_q_indices.iterator,
                cute.group_modes(
                    cute.select(qbucket_task_q_indices.layout, mode=[2, 1, 0]),
                    1,
                    3,
                ),
            )
        ...
        num_splits = (
            cute.size(qbucket_task_offsets.shape[1]) if const_expr(self.qbucket) else 1
        )
        ...
        if warp_idx < 4:
            ...
            if const_expr(self.wg_specialized_pipeline):
                ...
                if warp_idx_in_mma < 4:
                    ...
                    if const_expr(self.dKV_rs_split):
                        self.mma_wg1_qk_dv(*mma_args)
                    else:
                        self.mma_wg1_qk_dov_dq(*mma_args)
                else:
                    cute.arch.setmaxregister_increase(self.num_mma_regs_wg1)
                    if const_expr(self.dKV_rs_split):
                        self.mma_wg2_dov_dk_dq(*mma_args)
                    elif const_expr(self.dKV_rs_wg1):
                        self.mma_wg2_pds_sync(*mma_args)
                    else:
                        self.mma_wg2_dkv(*mma_args)
            elif const_expr(self.num_wg_dQ == self.num_wg_mma):
                # Both WGs compute dQ
                cute.arch.setmaxregister_increase(self.num_mma_regs_wg0)
                self.mma(*mma_args, is_dQ_wg=True)
            else:
                # First MMA WG computes dQ, second MMA WG skips it.
                warp_idx_in_mma = cute.arch.make_warp_uniform(cute.arch.warp_idx()) - 4
                if warp_idx_in_mma < 4:
                    cute.arch.setmaxregister_increase(self.num_mma_regs_wg0)
                    self.mma(*mma_args, is_dQ_wg=True)
    ...
    def load(
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                if const_expr(self.qbucket):
                    task_start = qbucket_task_offsets[n_block, q_group, (head_idx, batch_idx)]
                    task_end = qbucket_task_offsets[n_block + 1, q_group, (head_idx, batch_idx)]
                    total_m_block_cnt = task_end - task_start
                ...
                if process_tile:
                    if const_expr(self.qbucket or not self.use_block_sparsity):
                        first_m_block = (
                            qbucket_task_q_indices[task_start, (head_idx, batch_idx)]
                            if const_expr(self.qbucket)
                            else m_block_min
                        )
                        ...
                        for iter_idx in cutlass.range(1, total_m_block_cnt, unroll=1):
                            m_block = (
                                qbucket_task_q_indices[
                                    task_start + iter_idx, (head_idx, batch_idx)
                                ]
                                if const_expr(self.qbucket)
    ...
    def mma_wg1_qk_dov_dq(
        ...
        if const_expr(self.dKV_rs_wg1):
            ...
            acc_dK, _, tdKrQt = sm90_utils.partition_fragment_ABC(
                wg_mma_dK, shape_mnk_dK, None, sQt, swap_AB=False
            )
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(self.qbucket):
                task_start = qbucket_task_offsets[n_block, q_group, (head_idx, batch_idx)]
                task_end = qbucket_task_offsets[n_block + 1, q_group, (head_idx, batch_idx)]
                loop_count = task_end - task_start
            ...
            if process_tile:
              for iter_idx in cutlass.range(loop_count, unroll=1):
                ...
                if const_expr(self.V_in_regs):
                    if pds_iter == 0:
                        cute.copy(smem_thr_copy_KV, tSsK, tSrK_copy_view)
                ...
                if const_expr(not self.skip_score_mask):
                    if const_expr(
                        mBlockSizes is not None
                ...
                if const_expr(not self.dKV_rs_wg1):
                ...
                cute.arch.fence_view_async_shared()
                cute.arch.barrier_arrive(
                    barrier_id=int(NamedBarrierBwd.dSReady) + smem_idx_PdS,
                    number_of_threads=self.num_mma_threads,
                )
                if const_expr(self.dKV_rs_wg1):
                    gemm_w_idx(
                ...
                smem_idx_dQaccum = (
                    pds_iter % self.dQaccum_stage if const_expr(self.dQaccum_stage > 1) else 0
                )
                cute.arch.barrier(
                    barrier_id=int(NamedBarrierBwd.dQEmptyWG0) + smem_idx_dQaccum,
                    number_of_threads=self.num_threads_per_warp_group
                    + self.num_dQ_store_warps * cute.arch.WARP_SIZE,
                )
                sdQaccum_cur = (
                    sdQaccum[None, None, smem_idx_dQaccum]
                    if const_expr(self.dQaccum_stage > 1)
                    else sdQaccum
                )
    ...
    def mma_wg1_qk_dv(
        ...
        if const_expr(self.V_in_regs):
            ...
            smem_thr_copy_K = utils.make_tiled_copy_B(
                smem_copy_atom_K, tiled_mma_SdP, swapAB=self.SdP_swapAB
            ).get_slice(wg_tidx)
    ...
    def mma_wg2_dov_dk_dq(
        ...
        if const_expr(self.V_in_regs):
            ...
            smem_thr_copy_V = utils.make_tiled_copy_B(
                smem_copy_atom_V, tiled_mma_SdP, swapAB=self.SdP_swapAB
            ).get_slice(wg_tidx)
            ...
            if process_tile:
                for iter_idx in cutlass.range(loop_count, unroll=1):
                    ...
                    for r in cutlass.range_constexpr(cute.size(acc_dP_mn, mode=[0])):
                        ...
                        for c in cutlass.range(cute.size(acc_dP_mn, mode=[1]), unroll_full=True):
                            acc_dP_mn[r, c] = tdPrP_mn[r, c].to(Float32) * (
    ...
    def mma_one_m_block(
        ...
        if cutlass.const_expr(mask_fn is not None and not self.skip_score_mask):
            mask_fn(acc_S, m_block=m_block)
        acc_S_mn = layout_utils.reshape_acc_to_mn(acc_S, transpose=self.SdP_swapAB)
        lane_idx = cute.arch.lane_idx()
        for r in cutlass.range_constexpr(cute.size(acc_S_mn, mode=[0])):
    ...
    def epilogue_dKV_one_wg(
        ...
        epi_barrier = cutlass.pipeline.NamedBarrier(
            barrier_id=int(NamedBarrierBwd.Epilogue), num_threads=self.num_threads_per_warp_group
        )
        warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx())
    ...
    def epilogue_dV_one_wg(
        ...
        epi_barrier = cutlass.pipeline.NamedBarrier(
            barrier_id=int(NamedBarrierBwd.EpilogueV), num_threads=self.num_threads_per_warp_group
        )
        warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx())
    ...
    def epilogue_dK_one_wg(
        ...
        epi_barrier = cutlass.pipeline.NamedBarrier(
            barrier_id=int(NamedBarrierBwd.EpilogueK), num_threads=self.num_threads_per_warp_group
        )
        warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx())
    ...
    def dQaccum_store(
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile:
                if const_expr(self.qbucket or not self.use_block_sparsity):
                    assert not self.deterministic or not self.qbucket, (
                        "Deterministic not implemented for qbucket backward"
                    )
                    if const_expr(self.dQaccum_stage > 1):
                    ...
                    for iter_idx in cutlass.range(loop_count, unroll=1):
                        ...
                        for warp_group_idx in cutlass.range_constexpr(num_dQ_chunks):
                            if const_expr(
                                not self.deterministic and self.dQaccum_stage == 1
                            ):
                        ...
                        if const_expr(not self.deterministic and self.dQaccum_stage > 1):
                            if iter_idx >= self.dQaccum_stage:
                                cute.arch.cp_async_bulk_wait_group(
                                    self.dQaccum_stage - 1, read=read_flag
                                )
                                cute.arch.barrier_arrive(
                        ...
                        for warp_group_idx in cutlass.range_constexpr(num_dQ_chunks):
                            ...
                            sdQaccum_cur = (
                                sdQaccum[None, warp_group_idx, smem_idx_dQaccum]
                                if const_expr(self.dQaccum_stage > 1)
                                else sdQaccum[None, warp_group_idx]
                            )
                            with cute.arch.elect_one():
                                copy_utils.cpasync_reduce_bulk_add_f32(
