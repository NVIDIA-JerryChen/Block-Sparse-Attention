...
from csrc.bwd.sm90_blk64 import ampere_helpers as sm80_utils
...
class FlashAttentionBackwardPostprocess:
    def __init__(
        ...
        :type tile_m: int
        """
        assert arch // 10 == 9, "SM90 blk64 BSA postprocess only supports Hopper"
        assert head_dim == 128, "SM90 blk64 BSA postprocess is specialized for D=128"
        assert tile_m == 64, "SM90 blk64 BSA postprocess requires tile_m=64"
        assert num_threads == 256, "SM90 blk64 BSA postprocess expects two MMA WGs"
        assert AtomLayoutMdQ == 1, "SM90 blk64 BSA postprocess expects AtomLayoutMdQ=1"
        assert not dQ_swapAB, "SM90 blk64 BSA postprocess expects non-swapped dQ"
        assert not use_2cta_instrs and cluster_size == 1
        self.dtype = dtype
        self.tile_m = tile_m
        self.arch = arch
