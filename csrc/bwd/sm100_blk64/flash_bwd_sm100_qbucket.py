...
class BlockSparseAttnBackwardQRangeBucketed:
    def __init__(
        self,
    ...
    def _get_workspace_size(
        ...
        # Total bytes = B * H * (Q * (D + 2) + 2 * K * D) * acc_bytes.
        ...
        return (b, h, q * (d + 2) * acc_bytes + 2 * k * d * acc_bytes)
    ...
    def get_workspace_tensor(
        ...
        dK_acc_iter = dQ_acc_iter + cute.assume(B * H * Q * D * acc_bytes, divby=acc_bytes)
        dV_acc_iter = dK_acc_iter + cute.assume(B * H * K * D * acc_bytes, divby=acc_bytes)
    ...
    @staticmethod
    def _compute_bwd_grid(problem_shape, task_kv: cute.Tensor):
        H, B = problem_shape[3][0], problem_shape[3][1]
        tasks_per_group = cute.size(task_kv.shape[0])
        num_q_groups = cute.size(task_kv.shape[1])
    ...
    def __call__(
        ...
        # (b, h, q_group, task) -> (task, q_group, (h, b))
        task_kv = cute.make_tensor(
            task_kv.iterator,
            cute.group_modes(
                cute.select(task_kv.layout, mode=[3, 2, 1, 0]),
                2, 4
            )
        )
        ...
        task_offsets = cute.make_tensor(
            ...
            cute.group_modes(
                cute.select(task_offsets.layout, mode=[3, 2, 1, 0]),
                2, 4
            )
        )
        # (b, h, edge) -> (edge, (h, b))
        task_q_indices = cute.make_tensor(
            task_q_indices.iterator,
            cute.group_modes(
                cute.select(task_q_indices.layout, mode=[2, 1, 0]),
                1, 3
            )
        )
        ...
        dQ_smem_layout_staged = cute.tile_to_shape(
            dQ_smem_layout_atom,
            (self.QK_mma_tiler[0], 32, self.reduce_tma_store_stage),
            order=(1, 0, 2)
        )
        ...
        bwd_grid = self._compute_bwd_grid(problem_shape, task_kv)
        self.bwd(
            QK_tiled_mma,
            fake_QK_tiled_mma,
            dOV_tiled_mma,
    ...
    def bwd(
        ...
        kv_block_idx = task_kv[task_idx, q_group, (bidy, bidz)]
        task_start = task_offsets[task_idx, q_group, (bidy, bidz)]
        task_end = task_offsets[task_idx + 1, q_group, (bidy, bidz)]
        iter_count = task_end - task_start
        iter_index = Int32(0)
        ...
        if kv_block_idx >= 0 and kv_block_idx * self.QK_mma_tiler[1] < seqlen_k and iter_count > 0:
            if warp_idx == self.load_warp_id:
    ...
    def load(
        ...
        q_block_idx_0 = task_q_indices[task_start + iter_index, (blk_coord_h, blk_coord_b)]
        iter_index += 1
        q_block_idx_1 = seqlen_q // self.sparse_block_size # out of box, tma can fill zeros automatically
        if iter_index < total_iter_count:
            q_block_idx_1 = task_q_indices[task_start + iter_index, (blk_coord_h, blk_coord_b)]
        ...
        for i in cutlass.range_constexpr(async_copy_num_elts):
            sum_OdO_idx = (
                q_block_idx_1 * self.sparse_block_size + thread_idx * async_copy_num_elts
            )
            if cute.elem_less(sum_OdO_idx + i, seqlen_q):
                cute.copy(
                    atom_async_copy,
    ...
    def reduce(
        ...
        sdQ = cute.make_tensor(
            sdQ.iterator,
            cute.make_layout(((8,8),2,(32,1),(1,2)), stride=(((32,256),2048,(1,0),(0,4096))))
        )
