...
struct CollectiveEpilogueFwd {
    ...
    // the store path keeps the validated 4D descriptor shape.
    ...
    // 4D TMA store: (seqlen_q, kOutputCols, H_or_SplitH, B).
    static TMA_O make_tma_store_O(bsa_fwd_params const& p) {
        using namespace cute;
        auto shape_o  = make_shape(p.seqlen_q, Int<kOutputCols>{}, p.h * p.kv_splits, p.b);
    ...
    static ShapeO4 make_shape_O(bsa_fwd_params const& p) {
        using namespace cute;
        return make_shape(p.seqlen_q, Int<kOutputCols>{}, p.h * p.kv_splits, p.b);
    }
    ...
    CUTLASS_DEVICE EpiState tma_store(
            ...
            int head, int row_tile, int batch, int split, int num_row_tiles,
            EpiState state)
    {
        using namespace cute;
        ...
        if (elect_one_sync()) {
            ...
            if constexpr (HasKvSplits) {
                folded_head = split * params.fwd.h + head;
            }
            Tensor gO_hb = gO_full(_, _, folded_head, batch);
