...
from block_sparse_attention.csrc.utils.tcgen05_mma_helpers import (
    _tcgen05_mma_kind,
    i64_to_i32x2,
)
...
def tmem_load_red_max_32dp32b32x(
    ...
    values = tuple(
        Float32(llvm.extractvalue(T.f32(), out, [i])) for i in range(32)
    )
...
def tmem_load_red_max_32dp32b16x(
    ...
    values = tuple(
        Float32(llvm.extractvalue(T.f32(), out, [i])) for i in range(16)
    )
