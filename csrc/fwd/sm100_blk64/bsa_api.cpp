...
//
// Layout convention: BSHD = (batch, seqlen, num_heads, head_dim) with head_dim
// stride-1 (innermost). The kernel is BSHD-only; callers holding BHSD tensors
// must permute to BSHD before invoking (DSA-style — normalize at the boundary,
// kernel interior has a single canonical layout).
...
// BSHD-only: stride(1) is the seqlen stride, stride(2) is the head stride.
void set_params_fprop(bsa_fwd_params &params,
    ...
    // BSHD: axis 0=batch, 1=seq, 2=head, 3=dim (stride 1).
...
// Entry point: BSHD-only. Caller holding BHSD must permute to BSHD first.
        ...
        torch::Tensor q2k_block_nums)
{
    ...
    TORCH_CHECK(h == k.size(2) && h == v.size(2),
                "blk64 requires MHA: q/k/v must share num_heads (size(2))");
    TORCH_CHECK(seqlen_k == v.size(1), "k/v seqlen mismatch");
    ...
    auto out = torch::empty({b, seqlen_q, h, kOutputCols}, q.options());
...
// Tensors must be BSHD: (batch, seqlen, num_heads, head_dim).
