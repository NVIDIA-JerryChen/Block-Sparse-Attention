"""SM120 Sage INT8 QK and FP8 PV block-sparse attention forward kernel."""
...
class BlockSparseAttnForwardSageSm120Blk64(BatchedStaticSchedulerMixin):
    ...
    def kernel(
        ...
        gVScale = mVScale[
            None, work_desc.kv_head_idx, work_desc.batch_idx
        ]
        gO = cute.local_tile(
            mO_slice,
    ...
    def __call__(
        ...
        class SharedStorage:
            ...
            O_smem: cute.struct.Align[
                cute.struct.MemRange[
                    self.O_dtype, cute.cosize(self.O_smem_layout)
                ],
                128,
            ]
...
def _rescale_o_for_next_acc_fp8(
    ...
    for m in cutlass.range(cute.size(prev_ratio_m), unroll_full=True):
        should_rescale = (
            cute.arch.vote_ballot_sync(prev_ratio_m[m] < 1.0) != 0
        )
        if should_rescale:
            tOrO_mn[m, None].store(
                tOrO_mn[m, None].load() * prev_ratio_m[m]
            )
